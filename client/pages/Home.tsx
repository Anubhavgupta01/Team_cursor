...
import { Bot, ShieldCheck, Sparkles, Rocket, CreditCard, HelpCircle, Mail, Github, Twitter, Check } from "lucide-react";
    ...
    <section className="relative overflow-hidden py-16 md:py-24 animate-fade scroll-mt-16" id="home">
      ...
      <div className="mx-auto max-w-5xl text-center">
        ...
        <p className="mt-4 text-lg text-muted-foreground max-w-3xl mx-auto">
          Chat, create, and discover with an elegant interface that feels familiar—fast, simple, and delightful.
        </p>
        <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-3">
          <a href="/" className="w-full sm:w-auto"><Button className="h-12 px-8 bg-brand-gradient text-white text-base">Try Now</Button></a>
          <a href="#features" className="w-full sm:w-auto"><Button variant="outline" className="h-12 px-8 text-base">Explore Features</Button></a>
        </div>
      </div>
    </section>
  );
...
function Features() {
  const items = [
    { icon: Bot, title: "Smart Generation", desc: "Create stories and images powered by AI, tailored to your style." },
    { icon: ShieldCheck, title: "Private by default", desc: "Your content stays on your device unless you choose to share." },
    { icon: Rocket, title: "Fast & Fluid", desc: "Subtle animations and instant feedback keep you in flow." },
...
function Pricing() {
  const plans = [
    { name: "Free", price: "$0", features: ["5 stories/month", "Basic export", "Standard styles"], cta: "Get Started" },
    { name: "Pro", price: "$12", features: ["Unlimited stories", "HD images", "PDF & Gallery export"], cta: "Go Pro", highlight: true },
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {plans.map((p) => (
            <Card key={p.name} className={`${p.highlight ? "ring-2 ring-primary" : ""}`}>
              <CardHeader>
                <CardTitle className="text-2xl flex items-center gap-2">{p.name} {p.highlight && <CreditCard className="h-5 w-5 text-primary"/>}</CardTitle>
                <div className="text-3xl font-extrabold">{p.price}<span className="text-base text-muted-foreground">/mo</span></div>
              </CardHeader>
              <CardContent>
                ...
                <a href="/signup"><Button className={`${p.highlight ? "bg-brand-gradient text-white" : ""} w-full`}>{p.cta}</Button></a>
              </CardContent>
            </Card>
          ))}
        </div>
      ...
      <div className="mx-auto max-w-3xl">
        ...
        <Accordion type="single" collapsible className="w-full">
          <AccordionItem value="q1">
            ...
            <AccordionContent>
              Enter an idea, choose options, and we create scenes with text and images. You can edit or regenerate any part.
            </AccordionContent>
          </AccordionItem>
          <AccordionItem value="q2">
            <AccordionTrigger>Can I export my stories?</AccordionTrigger>
            <AccordionContent>
              Yes—export to PDF or download the image gallery from the story view.
            </AccordionContent>
          </AccordionItem>
          <AccordionItem value="q3">
            <AccordionTrigger>Is there a free plan?</AccordionTrigger>
            <AccordionContent>
              The Free plan lets you try essential features with monthly limits. Upgrade anytime.
            </AccordionContent>
          </AccordionItem>
        </Accordion>
      </div>
    ...
    <section id="contact" className="py-16 md:py-24 animate-fade scroll-mt-16">
      <div className="mx-auto max-w-3xl text-center">
        ...
        <div className="mt-6 flex items-center justify-center gap-3">
          <a href="mailto:support@example.com"><Button><Mail className="h-4 w-4"/> Email</Button></a>
          <a href="https://twitter.com" target="_blank" rel="noreferrer"><Button variant="outline"><Twitter className="h-4 w-4"/> Twitter</Button></a>
          <a href="https://github.com" target="_blank" rel="noreferrer"><Button variant="outline"><Github className="h-4 w-4"/> GitHub</Button></a>
        </div>
      </div>
    </section>
  );
    ...
    <footer className="py-10 border-t mt-10 text-sm">
      <div className="container flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-2 text-muted-foreground">
          <Bot className="h-4 w-4"/> <span>StoryVisualizer</span> <span>© {new Date().getFullYear()}</span>
        </div>
        <div className="flex items-center gap-5 text-muted-foreground">
          <a className="hover:text-foreground" href="#home">Home</a>
